...
typedef td_err_e (td_set_tdb_verbosity_ftype) (int n);
typedef int (td_get_tdb_verbosity_ftype) (void);

td_set_tdb_verbosity_ftype *td_set_tdb_verbosity_p;
td_get_tdb_verbosity_ftype *td_get_tdb_verbosity_p;
...
struct thread_db_info
{
  ...
  /* MPC BEGIN */
  td_thr_getfpregs_ftype *td_thr_getfpregs_p;
  td_thr_setfpregs_ftype *td_thr_setfpregs_p;
  td_thr_getgregs_ftype *td_thr_getgregs_p;
  td_thr_setgregs_ftype *td_thr_setgregs_p;
  /* MPC END */
};
...
thread_from_lwp (ptid_t ptid)
{
  ...
  if (tdb_debug && err == TD_NOTHR)
    return NULL;	/* Thread must have terminated.  */
  if (err != TD_OK)
...
#undef CHK
  ...
  if (info->td_ta_thr_iter_p == NULL)
  ...
  else if (thread_db_find_new_threads_silently (inferior_ptid) != 0)
    {
      ...
      if (err != TD_OK)
	{
	  /* It doesn't actually work yet (probably libpthread.so hasn't
	     finished loading). */
	  return 0;
	}
    }
...
#define TDB_VERBOSE_DLSYM(info, func)			\
  info->func ## _p = (func ## _ftype *) verbose_dlsym (info->handle, #func)
	...
	TDB_VERBOSE_DLSYM (info, td_thr_getgregs);
	...
	TDB_VERBOSE_DLSYM (info, td_thr_setgregs);
	...
	TDB_VERBOSE_DLSYM (info, td_thr_getfpregs);
	...
	TDB_VERBOSE_DLSYM (info, td_thr_setfpregs);
	...
	td_set_tdb_verbosity_p
	  = (td_set_tdb_verbosity_ftype *) dlsym(handle, "td_set_tdb_verbosity");
	td_get_tdb_verbosity_p
	  = (td_get_tdb_verbosity_ftype *) dlsym(handle, "td_get_tdb_verbosity");
      }
    return 1;
  }
...
thread_db_pid_to_str (struct target_ops *ops, ptid_t ptid)
{
  ...
  if (thread_info != NULL && thread_info->priv != NULL)
	...
	snprintf (buf, sizeof (buf), "Thread 0x%lx (LWP %d.%ld)",
		  (unsigned long) tid, ptid_get_pid (ptid), ptid_get_lwp (ptid));
      else
