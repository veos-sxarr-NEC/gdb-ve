...
upc_read_thread_sym (char *sym_name)
{
  ...
  bs = lookup_symbol (sym_name, 0, VAR_DOMAIN, NULL);
  mythread_sym = bs.symbol;
  if (mythread_sym)
    {
      ...
      mythread_val = read_var_value (mythread_sym, bs.block, frame);
      if (mythread_val)
...
upc_enable_thread_debug (void)
{
  ...
  if (!upc_thread_active)
	...
	return;
      ...
      bmsym = lookup_minimal_symbol ("THREADS", NULL, NULL);
      if (bmsym.minsym == NULL)
	{
      ...
      bmsym = lookup_minimal_symbol ("MYTHREAD", NULL, NULL);
      if (bmsym.minsym == NULL)
	{
      ...
      bmsym = lookup_minimal_symbol ("UPC_PTHREADS", NULL, NULL);
      if (bmsym.minsym != NULL)
	{
...
upc_thread_attach (struct thread_info *t)
{
  ...
  if (!upc_monitor)
      ...
      if (upcstartgate)
	{
	  ...
	  gate_addr = BMSYMBOL_VALUE_ADDRESS (bmsym);
  ...
  if (!upc_pthread_active && (thread_count () != 1))
    {
      ...
      delete_inferior (upc_monitor_inferior);
