...
upc_main_name (void)
{
  ...
  bmsym = lookup_minimal_symbol (UPC_MAIN_PROGRAM_SYMBOL_NAME, NULL, NULL);
...
upc_shared_var_address (struct symbol *var)
{
  ...
  if ((SYMBOL_CLASS (var) == LOC_STATIC) && !overlay_debugging)
  ...
  else if (SYMBOL_CLASS (var) == LOC_UNRESOLVED)
    {
      ...
      if (bmsym.minsym == NULL)
          return shared_addr;
      sym_addr = BMSYMBOL_VALUE_ADDRESS (bmsym);
    }
...
struct value *
upc_read_var_value (struct symbol *var, const struct block *var_block,
...
upc_lang_init (char *cmd, int from_tty)
{
  ...
  if (status == uda_ok && num_threads > 0)
  ...
  else
    {
      ...
      /* UPC program? */
      bs = lookup_symbol ("THREADS", 0, VAR_DOMAIN, NULL);
      threads_sym = bs.symbol;
      if (threads_sym)
        {
          threads_val = read_var_value (threads_sym, bs.block, NULL);
          if (threads_val)
